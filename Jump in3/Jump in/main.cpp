...
void desenareTabla()
{
    int i,j;
    setcolor(WHITE);
    setfillstyle(SOLID_FILL,COLOR(16,164,54));
    for (i=0; i<DIM; i++)
        for (j=0; j<DIM; j++)
    ...
    */
    ...
    for(int k=0; k<nrMutari; k++)
        if(TablaDeJoc[PozitiiAccesibile[k].i][PozitiiAccesibile[k].j]==2)
            readimagefile("red_hole.jpg",stanga+latura*(PozitiiAccesibile[k].i)+2,sus+latura*(PozitiiAccesibile[k].j)+2,stanga+latura*(PozitiiAccesibile[k].i+1)-2,sus+latura*(PozitiiAccesibile[k].j+1)-2);
        else
           readimagefile("red_rectangle.jpg",stanga+latura*(PozitiiAccesibile[k].i)+2,sus+latura*(PozitiiAccesibile[k].j)+2,stanga+latura*(PozitiiAccesibile[k].i+1)-2,sus+latura*(PozitiiAccesibile[k].j+1)-2);
}
bool JocCastigat()
{
...
main()
{
    PlaySound(TEXT("da.wav"),NULL,SND_ASYNC);
